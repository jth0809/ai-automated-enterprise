...
Prometheus→Alertmanager→Discord 경로는 2026-07-18 실제 경고 8건과 RESOLVED 수신으로 확인했다: `Alertmanager runtime delivery verified 2026-07-18`. Flux Provider 직접 경로는 notification-controller 복구 후, Flagger AlertProvider 직접 경로는 다음 자연 카나리 이벤트에서 확인할 때까지 각각 `runtime delivery verification pending`이다. 프로덕션 장애를 인위적으로 만들지 않으며, 프로비저닝·그룹 정책·복구 판정·교체 절차는 [Discord 알림 운영 Runbook](docs/runbooks/discord-alerting.md)을 따른다.
...
| 운영 알림 (Discord) | 🛠️ GitOps 구성 완료. `Alertmanager runtime delivery verified 2026-07-18`; Flux·Flagger 직접 경로는 `runtime delivery verification pending`. 전역 `AlertmanagerConfig`는 info·구조적 overcommit을 제외하고 warning을 네임스페이스별로 묶음 |

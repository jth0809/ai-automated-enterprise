...
WP2.1~WP2.6의 소프트웨어·데이터 증거는 완결됐다. **G2는 2026-07-14 인간
(사용자) 승인으로 닫혔다.** LIVE_MODEL은 사용자 결정에 따라 계속
`NOT_ACTIVATED`로 유지하며, 활성화는 향후 CNP allowlist + 보안 검토를 거쳐
별도로 결정한다. PR CI 보안 스캔(Semgrep/Trivy/Gitleaks)은 PR에서 실행한다.
...
- ✅ **인간(사용자) G2 승인**: 2026-07-14.
- ✅ **LIVE_MODEL 결정**: 사용자 결정으로 미활성 유지(`NOT_ACTIVATED`).
- ⏳ **PR CI 보안 스캔**: PR에서 Semgrep/Trivy/Gitleaks 실행 후 확인.

G2 통과로 Phase 3(앵커·관측) 실행 전제가 충족됐다. 시스템은 데이터를
축적하며 Phase 3 개발과 병행한다.

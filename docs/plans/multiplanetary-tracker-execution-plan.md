...
- [x] **WP2.1 기술 트리 확정** — 노드 ~35개 + 의존성 간선 + 가중치: LLM 배치 초안 → AHP 쌍대 비교 → 인간 검수 확정(12.1 절차). 루브릭 버전으로 관리. **통합 실증 노드에 finish line 등치 정점("26개월 무보급 연속 운영" — 컨셉 v2.11) 포함 필수**
  - **상태(2026-07-14):** 사용자 승인 `nodes-v1.0` 35개 노드·29개 필수 간선·
    필라별 가중치 합 1.0·통합 노드 6개와 `r2.0` registry/해시 계약을
    폐쇄 감사해 `TrackerNodesV1Test` 13/13 통과, 구조를 동결했다 (`305734b`).
  - **잔여 범위:** 구조 산출물과 35노드 현재상태 감사는 확정했지만 G2는 열어
    둔다. AHP 재보정은 현행 값을 역산하지 않고 새 노드 버전으로만 제안한다.
- [x] **WP2.2 히스토리컬 백필** — 1957~현재 주요 마일스톤 수백 건, Batch API 초안 + 인간 검수, 초기 TRL/EGL·휴면 감사(아폴로 계열 휴면 처리 포함)
  - **완료(2026-07-14):** 후보 213·청구 147의 저작권 안전 참조형 코퍼스로
    35/35 노드의 현재 level/status를 감사했다. 1957-01-07~2026-07-13의
    3,628주를 6개 필라 21,768행으로 멱등 투영하고 마지막 주와 fresh DB의
    감사 상태를 일치시켰다. 집중 45/45·전체 백엔드 295/295·GitOps egress
    정책 검증을 통과했다. 상세 근거와 절차는 아래 WP2.2-B/C 계획 및 주간
    백필 런북에 둔다.
...
- 승인 설계: [Phase 2 전체 설계](../superpowers/specs/2026-07-14-tracker-phase2-design.md)
...
- 완료: [WP2.2-B/C 전수 감사·주간 백필](../superpowers/plans/2026-07-14-tracker-phase2-wp22b-wp22c-plan.md)
  — 35/35 감사와 1957~현재 주간 투영 완료. 검증 절차는
  [주간 백필 런북](../runbooks/tracker-weekly-backfill-validation.md)에 기록.
...
- [x] **WP4.2 수학 정교화** — 사건 수 기반 수축(k=4), 필라별 W_p 규칙(m=6, 4~15년), 개입 분석(수준 이동 더미), 구조 단절(윈도우 리셋), Holt는 UI 모멘텀 전용으로 분리 ([상세 계획](../superpowers/plans/2026-07-16-tracker-wp42-complete-trend-plan.md), [검증 증거](../research/tracker-wp42-validation-evidence.md))
  - 완료 기준: 계단형 합성 데이터에서 가짜 모멘텀 부재 확인(점프 직후 ETA 요동 < 지정 임계)
  - **완료(2026-07-16):** V17 `params-v2`, 역사 DAG 재생, cutoff-safe
    적응형 창, rollback 단계 더미, 6필라 부분 풀링과 원자적 snapshot 감사를
    구현했다. Maven 650/650, frontend 79/79와 build, 실제 147건 백필 API·브라우저
    console 0을 통과했다. 현재 잔차 구간은 WP4.3에서 몬테카를로로 교체한다.
- [x] **WP4.3 몬테카를로** — 회귀 계수·w_c·매핑·보정 파라미터(δ_e, k, 휴면 곡선) 섭동, 중앙값+80% 구간 산출로 잔차 구간 대체 ([상세 계획](../superpowers/plans/2026-07-16-tracker-wp43-monte-carlo-plan.md), [검증 증거](../research/tracker-wp43-validation-evidence.md))
  - **완료(2026-07-16):** 결정론적 4,000회 표본, 제약 보존 섭동, 무조건부 오른쪽 검열 분위수, 입력 해시별 원자적 7행 저장을 구현했다. 실제 147건에서 invalid 0/4,000, 전체 p50 2091.2와 모델 내 80% 구간 2089.1–2093.6을 확인했으며 기본 플래그는 계속 비활성이다.
- [x] **WP4.4 백테스트 하네스 + 파라미터 보정** — 시점 절단 hindcast, **체제 경계 홀드아웃**(~2010 보정 / 2010~ 검증), m·k·δ_e 보정(W_p는 검증만) ([상세 계획](../superpowers/plans/2026-07-16-tracker-wp44-backtest-plan.md), [검증 증거](../research/tracker-wp44-validation-evidence.md))
  - **완료(2026-07-16):** 27개 사전 선언 후보를 52개 보정 cutoff에서만 비교해 `m=8, k=4, δ=0.75`를 잠갔고, 16개 홀드아웃 cutoff·408개 fold·35개 지표를 공개했다. 파일 DB 재시작에서도 run ID·입력/보고서 hash·seed가 재사용됐다.
- [x] **WP4.5 마이크로 예측 + Brier + 라이브 캘리브레이션** — **확률 생성 규칙 정의가 첫 태스크**(사건 빈도 기반 위험률(hazard rate) 모델 — 컨셉 v2.10에서 위임된 미명세 갭), 6~24개월 예측 자동 생성·공표, 만기 처리, Brier 트랙 레코드 공개, 확률 재보정 레이어 + 드리프트 진단(구조 파라미터 직접 최적화 금지 — 굿하트 규칙) ([상세 계획](../superpowers/plans/2026-07-16-tracker-wp45-micro-prediction-plan.md), [첫 코호트 증거](../research/tracker-wp45-validation-evidence.md))
  - **완료(2026-07-16):** 필라당 2개·총 12개의 결정론적 24개월 코호트를 발행하고 동일 입력 재발행의 멱등성을 입증했다. 결과 0건·0분기이므로 보정과 Brier는 정직하게 `INSUFFICIENT_DATA`이며 자동 발행·만기 처리는 비활성이다.
- [x] **WP4.6 방법론 투명성 페이지** — 수식·규칙·레이어 발산 현황·예측 실적·데이터 소스 전체 공개, 도달 판정·이정표 승계 규칙 게시, **정직성 표기 4건 필수 게시**(컨셉 v2.10 — ① ETA=시나리오 투영·모델 내 80%, ② $/kg=공표 가격 기반, ③ 측정 vs 구성 지수 구분, ④ 수송 경제성 임계값=선언된 가정) ([상세 계획](../superpowers/plans/2026-07-16-tracker-wp46-transparency-plan.md), [검증 증거](../research/tracker-wp46-validation-evidence.md))
  - **완료(2026-07-18):** 6개 읽기 API, 정직성 문구 4건, DAG·투영·백테스트·12개 예측·Brier 상태 화면을 구현했다. Maven 774/774, Vitest 84/84, build, 1280/375px, console 0, egress·GitOps와 [PR #43](https://github.com/jth0809/ai-automated-enterprise/pull/43)의 Gitleaks·Semgrep·Trivy를 통과했다. 정식 런칭은 실제 14일 관찰 뒤에만 완료한다.

**게이트 G4 (= 정식 런칭):** 백테스트 리포트 공개, 첫 마이크로 예측 세트 공표, 투명성 페이지 가동, 운영 런북 기준 2주 무개입 안정 운영.

**현재 상태:** `PENDING_OBSERVATION`. Phase 4 소프트웨어 게이트와 PR 보안
검사는 통과했지만 실제 14일 관찰은 아직 완료되지 않았다.
관찰 절차는 [G4 런북](../runbooks/tracker-phase4-g4-observation.md)을 따른다.

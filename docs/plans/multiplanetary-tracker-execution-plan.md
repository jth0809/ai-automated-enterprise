...
- [ ] **WP4.6 방법론 투명성 페이지** — 수식·규칙·레이어 발산 현황·예측 실적·데이터 소스 전체 공개, 도달 판정·이정표 승계 규칙 게시, **정직성 표기 4건 필수 게시**(컨셉 v2.10 — ① ETA=시나리오 투영·모델 내 80%, ② $/kg=공표 가격 기반, ③ 측정 vs 구성 지수 구분, ④ 수송 경제성 임계값=선언된 가정) ([상세 계획](../superpowers/plans/2026-07-16-tracker-wp46-transparency-plan.md), [로컬 검증 증거](../research/tracker-wp46-validation-evidence.md))
  - **로컬 완료·PR 보안 게이트 대기(2026-07-16):** 6개 읽기 API, 정직성 문구 4건, DAG·투영·백테스트·12개 예측·Brier 상태 화면을 구현했다. Maven 774/774, Vitest 84/84, build, 1280/375px, console 0, egress와 GitOps 렌더링은 통과했으며 PR의 Gitleaks·Semgrep·Trivy 결과 확인 전에는 WP4.6과 정식 런칭을 완료로 표시하지 않는다.

**게이트 G4 (= 정식 런칭):** 백테스트 리포트 공개, 첫 마이크로 예측 세트 공표, 투명성 페이지 가동, 운영 런북 기준 2주 무개입 안정 운영.

**현재 상태:** `PENDING_PR_SECURITY_AND_OBSERVATION`. 소프트웨어 로컬
게이트는 통과했지만 PR 보안 게이트와 실제 14일 관찰은 아직 완료되지 않았다.

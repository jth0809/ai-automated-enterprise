...
- [ ] **WP2.2 히스토리컬 백필** — 1957~현재 주요 마일스톤 수백 건, Batch API 초안 + 인간 검수, 초기 TRL/EGL·휴면 감사(아폴로 계열 휴면 처리 포함)
  - 완료 기준: 전 필라의 주간 스냅샷 시계열이 1957~현재 연속 생성되고,
    현재 상태가 감사 결과와 일치한다.
  - **WP2.2-A 완료(2026-07-14):** P1 8개 노드의 현재상태·휴면을 감사해
    후보 212/READY 212/REJECTED 0, 청구 146, 고유 후보 140, P1 청구 32,
    준비도 `0.4624`를 고정했다. 집중 45/45·독립 코퍼스 42/42·전체 백엔드
    280/280과 GitOps egress 정책 검증을 통과했다 (`8d3c91b`, `af43164`).
  - **WP2.2-B 완료(2026-07-14):** P2~P6 27개를 포함한 35/35 감사 행렬을
    후보 213·청구 147의 참조형 코퍼스와 대조했다. HEALTH-AUTONOMY와
    THERMAL은 직접 근거 범위의 L3, RESOURCE/SETTLEMENT-INTEGRATION은
    직접 통합 근거 부재로 L0을 유지했고, focused 39/39가 통과했다.
  - **잔여 범위:** 실제 도달 조건·점수/ETA 수학은 동결하며 유한 ETA를 완료
    조건으로 삼지 않는다. 근거는 원문 없이 참조형 메타데이터만 저장한다.
    1957~현재 주간 스냅샷 생성과 WP2.3 이후 작업이 후속 범위다.
...
- 실행 중: [WP2.2-B/C 전수 감사·주간 백필](../superpowers/plans/2026-07-14-tracker-phase2-wp22b-wp22c-plan.md)
  — B 전수 감사 완료, C 주간 투영 진행 예정.

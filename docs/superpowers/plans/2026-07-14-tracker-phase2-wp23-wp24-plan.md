...
> 상태: 완료 (2026-07-14)
>
> 검증 증적: [tracker-phase2-wp23-wp24-evidence.md](../../research/tracker-phase2-wp23-wp24-evidence.md)
...
- [x] 먼저 schema test를 작성해 새 table, column, FK, unique, enum check와 인덱스를 요구한다.
- [x] body column comment가 외부 기사 스냅샷이 아니라 합성·재서술 입력임을 요구한다.
- [x] golden result에 source body 또는 raw model output 열이 없음을 검사한다.
- [x] test를 실행해 V9 부재로 RED인지 확인한다.
- [x] Oracle과 H2 Oracle mode에서 호환되는 V9 migration을 작성한다.
- [x] 기존 비어 있는 golden_set_item을 안전하게 확장하고 모든 새 식별자를 bounded varchar로 둔다.
- [x] schema test와 전체 migration tests를 GREEN으로 만든다.
- [x] 커밋: feat(tracker): add Phase 2 quality schema
...
- [x] GoldenSetDatasetValidatorTest에 파일·필드·크기·금지 키·중복 case code 실패 테스트를 작성한다.
- [x] 6개 필라, state/non-state/rollback/cancellation/arrival, 관련 없음, quote 검증 실패, level 경계, duplicate/non-duplicate 표현쌍 분포를 검사하는 테스트를 작성한다.
- [x] RED를 확인한다.
- [x] golden-set-v1.json에 직접 작성한 50개 짧은 합성·재서술 case를 추가한다.
- [x] 기존 승인 historical claim ID만 provenance ref로 사용하고 외부 문장을 복제하지 않는다.
- [x] validator를 구현해 schema와 UTF-8 byte 상한을 강제한다.
- [x] GoldenSetLoader를 구현해 dataset hash가 같은 재실행을 no-op으로 만든다.
- [x] dataset 변경은 새 version을 요구하고 기존 version/hash 충돌을 실패시킨다.
- [x] fixture 파일 크기와 case별 최대 크기를 테스트 출력에 기록한다.
- [x] focused tests를 GREEN으로 만든다.
- [x] 커밋: data(tracker): add copyright-safe golden set

## Task 3: GoldenClassifier 포트와 canonical diff

**Files:** GoldenInput, GoldenOutput, GoldenClassifier, GoldenSetEvaluator와 tests

- [x] 다음 실패 테스트를 먼저 작성한다.
...
- [x] RED를 확인한다.
- [x] GoldenOutput canonicalizer와 field diff를 구현한다.
- [x] evaluator가 run/result를 한정된 레코드로 저장하게 repository를 확장한다.
- [x] OFFLINE_REPLAY용 fake classifier는 test source에만 둔다.
- [x] 실제 model adapter가 기존 DeepClassifier의 structured output 검증을 재사용하도록 최소 refactor한다.
- [x] DeepClassifier 기존 tests를 그대로 GREEN으로 유지한다.
- [x] 커밋: feat(tracker): evaluate golden agreement

## Task 4: 주간 골든 평가와 활성화 경계

**Files:** GoldenSetJob, DeepClassifierGoldenAdapter, TrackerConfig/application config, tests

- [x] scheduler가 기본 일요일 UTC cadence와 ShedLock을 가지는지 실패 테스트를 작성한다.
- [x] tracker.golden-live-enabled=false에서 AnthropicClient를 호출하지 않는 테스트를 작성한다.
- [x] LIVE_MODEL만 GOLDEN_LAST_LIVE_SUCCESS와 GOLDEN_LIVE_ACTIVATED를 갱신하는 테스트를 작성한다.
- [x] OFFLINE_REPLAY와 DRILL이 운영 baseline을 갱신하지 않는 테스트를 작성한다.
- [x] prompt/model/rubric/schema tuple 변경 시 새 baseline이 필요함을 테스트한다.
- [x] RED를 확인한다.
- [x] GoldenSetJob을 구현하고 batch limit 60을 강제한다.
- [x] 설정이 꺼졌을 때 SKIPPED 상태 또는 명시적 ops status만 기록하고 네트워크를 호출하지 않는다.
- [x] API 활성화 방법은 런북으로 미루고 평문 secret을 추가하지 않는다.
- [x] focused tests를 GREEN으로 만든다.
- [x] 커밋: feat(tracker): schedule versioned golden runs
...
- [x] RED를 확인한다.
- [x] StateFreezeService에 isFrozen, freeze, release를 구현한다.
- [x] freeze/release와 ops_action_log 쓰기를 transaction으로 묶는다.
- [x] StateUpdater가 scoring 및 최종 advance 직전에 동결을 재검사하도록 한다.
- [x] approve는 resolveReview보다 먼저 동결을 확인한다.
- [x] SnapshotJob의 displayed ETA 경로를 frozen-aware로 만든다.
- [x] 기존 review uniqueness와 idempotency를 유지한다.
- [x] focused tests를 GREEN으로 만든다.
- [x] 커밋: feat(tracker): freeze unsafe state transitions

## Task 6: 관리도

**Files:** ControlChart, PipelineMonitorJob, TrackerRepository와 tests

- [x] pure ControlChart 실패 테스트를 먼저 작성한다.
  ...
  - 비율 경계는 0~1
  - sigma=0에서 값 변화는 위반
- [x] repository 집계 테스트를 작성한다.
  - relevance gate pass rate
  - confirmed event count
  - impact median과 p95
  - 빈 날과 null score 처리
- [x] RED를 확인한다.
- [x] JDK-only 통계 계산을 구현한다.
- [x] daily job과 ShedLock을 추가하고 metric row를 upsert한다.
- [x] 두 번 연속 위반에서 StateFreezeService.freeze를 호출한다.
- [x] metric query와 저장은 최근 28일·고정 4개 metric으로 제한한다.
- [x] focused tests를 GREEN으로 만든다.
- [x] 커밋: feat(tracker): monitor pipeline control charts
...
- [x] RED를 확인한다.
- [x] 최근 bounded publication timestamp만 읽는 repository query를 추가한다.
- [x] median과 상태 계산을 pure class로 구현한다.
- [x] 결과를 ops_state 또는 bounded daily metric으로 노출한다.
- [x] 새 외부 alert endpoint나 egress를 만들지 않는다.
- [x] focused tests를 GREEN으로 만든다.
- [x] 커밋: feat(tracker): add feed deadman monitoring
...
- [x] DRILL fake가 44/50 agreement를 내도록 통합 테스트를 작성한다.
- [x] 자동 freeze, provisional 유지, approval 409 의미, 이유 없는 release 거부, 이유 있는 release, audit log, 단일 재처리를 한 테스트에서 검증한다.
- [x] DRILL이 GOLDEN_LAST_LIVE_SUCCESS를 바꾸지 않는지 검증한다.
- [x] 테스트를 GREEN으로 만든다.
- [x] 런북에 fixture 실행, 관찰 SQL/API, 해제 전 확인표와 rollback을 기록한다.
- [x] 커밋: test(tracker): prove circuit breaker drill

## Task 9: WP2.3/WP2.4 전체 검증과 문서

- [x] schema, golden, ops focused tests를 fresh 실행한다.
- [x] backend 전체 테스트를 fresh 실행한다.
- [x] fixture 크기, DB 행 수, scheduler count를 기록한다.
- [x] secret scan에서 token/API key fixture가 없음을 확인한다.
- [x] GitOps egress diff에서 새 도메인이 없음을 확인한다.
- [x] 마스터에는 WP2.3/WP2.4 요약과 이 상세 계획 링크만 추가한다.
- [x] .superpowers/sdd/progress.md에 로컬 실행 증거를 기록하되 스테이징하지 않는다.
- [x] 커밋: docs(tracker): record quality operations evidence

## 완료 체크

- [x] 저작권 안전 골든 case 50개
- [x] OFFLINE/LIVE/DRILL 명확한 분리
- [x] LIVE agreement 0.90 threshold 구현
- [x] 28일/14일/3 sigma/2회 관리도 구현
- [x] 2× median 데드맨 경보 구현
- [x] 자동 freeze와 인간 전용 release 구현
- [x] 훈련 통합 테스트 통과
- [x] 새 egress·평문 secret·별도 workload 0

...
- level이 1 이상이면 해당 level을 직접 지지하는 claim ID와 evidence ref
- level이 0이면 빈 level 근거와 level 1 직접 증거가 없다는 검색·경계 기록
...
- 최소 수량은 두지 않는다. 대신 별도 `backfill-audit-v1.json`에 35개 노드
  audit coverage가 모두 존재하고 청구·증거·재생 상태와 일치해야 한다.

이 상한은 데이터 부풀리기를 장려하지 않으면서 현 213개 후보와 후속 공식 근거를 충분히 수용한다.

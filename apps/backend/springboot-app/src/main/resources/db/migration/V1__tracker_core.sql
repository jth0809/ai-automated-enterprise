-- V1__tracker_core.sql
--
-- Multiplanetary Tracker core schema. Source of truth for column/table
-- shape: docs/plans/wp/tracker-data-model.md (DDL 초안, Phase 1 코어 +
-- 후속 페이즈 테이블 — all created here per WP1.1 decision: schema churn
-- is the expensive kind of change, so later-phase tables are pre-created).
--
-- Adaptations from the design doc (H2 MODE=Oracle test target / ATP Oracle
-- prod target, both must accept this file byte-for-byte):
--   1. Every `JSON` column in the doc is declared CLOB here (doc's own
--      "구현 노트": JSON parsing happens at the app level via Jackson).
--   2. Every `DEFAULT SYSTIMESTAMP` becomes `DEFAULT CURRENT_TIMESTAMP`
--      (ANSI-portable spelling both H2 and Oracle accept).
--   3. All other Oracle types used verbatim (VARCHAR2, NUMBER, CHAR, DATE,
--      TIMESTAMP, CLOB, GENERATED ALWAYS AS IDENTITY, inline REFERENCES,
--      inline/table-level CHECK) — H2's Oracle compatibility mode accepts
--      these unchanged.
...
CREATE TABLE parameter_set (
  ...
  trl_map           CLOB NOT NULL,   -- {"1":0.03,...,"9":1.0}
  maturity_map      CLOB NOT NULL,   -- EGL용 (초기값 = trl_map과 동일 곡선, 백테스트 보정 대상)
  created_at        TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
);
...
CREATE TABLE article_classification (                   -- LLM 출력 원본 = event↔article 조인 겸용
  ...
  raw_output    CLOB NOT NULL,                          -- structured output 전문
  rubric_version_id NUMBER NOT NULL REFERENCES rubric_version(id),
  created_at    TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
);
